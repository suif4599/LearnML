...
class DecoderLayer(torch.nn.Module):
    def __init__(self, d_model, n_head, seq_len, dropout=0.1):
        ...
        self.__seq_len = seq_len
        self.__dropout = dropout
        self.masked_multi_head_attention = MultiHeadAttention(d_model, n_head, dropout, seq_len, True)
        self.add_and_norm1 = torch.nn.LayerNorm(d_model)
        self.encoder_decoder_attention = EncoderDecoderAttention(d_model, n_head, seq_len, dropout)
        self.add_and_norm2 = torch.nn.LayerNorm(d_model)
        self.feed_forward = torch.nn.Sequential(
            torch.nn.Linear(d_model, 2 * d_model),
            torch.nn.ReLU(),
