...
# filename = os.path.join(os.path.dirname(__file__), "model_save", "model_20250303223050_epoch2.pt")
# model = TransformerTranslator.load(filename, len(eng_vocab), len(chn_vocab), eng_len, chn_len, 512, 8, 6)

model = TransformerTranslator(len(eng_vocab), len(chn_vocab), eng_len, chn_len, 512, 8, 6, dropout=0.05)
...
while 1:
    ...
    if eng == "exit":
        break
    if not eng.endswith(".") or not eng.endswith("?") or not eng.endswith("!"):
        eng += "."
    chn = ''.join(chn_rev_map[i] for i in model.translate(tokenize(eng, UNK, eng_vocab, eng_len)))
    chn = chn.split("<eos>")[0].removeprefix("<sos>")
